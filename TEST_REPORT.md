...
| Dart tests | `flutter test` | ✅ **99 / 99 passed** |
| Pipeline tests | `python3 -m unittest tools.test_parsers` | ✅ **22 / 22 passed** |
| Line coverage | `flutter test --coverage` | **91.3%** (1612 / 1766 lines) |
...
`result_card`, `tabs`) — is **91.3% covered** (1612 / 1766 lines). The uncovered
...
`isStale` unit tests), the sparkline `CustomPainter` paint method, and the
external `launchUrl` / `showLicensePage` handlers
