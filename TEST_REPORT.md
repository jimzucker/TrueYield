...
| Dart tests | `flutter test` | ✅ **103 / 103 passed** |
| Pipeline tests | `python3 -m unittest tools.test_parsers` | ✅ **22 / 22 passed** |
| Line coverage | `flutter test --coverage` | **91.7%** (1669 / 1821 lines) |
...
| `test/charts_test.dart` | 5 | The pure `waterfallSteps` data prep for the total-return waterfall: the steps start at cost and end at the after-tax value, the signed deltas balance, and the Real G/L + capital-gains-tax steps appear only when a lot is sold. |
| `test/widget_test.dart` | 28 | UI / end-to-end flow via an injected mock `http.Client`: the six tabs, the qualifying & "does not qualify" result cards, the portfolio grid for lots, the editable per-distribution ROC column, the Info tab (coverage summary, tracked-fund list, CSV links), the Diagnostics scenarios, validation, the fetch-error messages (server / unknown symbol / network), and saved-input restoration. |
...
`result_card`, `tabs`) — is **91.7% covered** (1669 / 1821 lines). The uncovered
