...
| `test/widget_test.dart` | 30 | UI / end-to-end flow via an injected mock `http.Client`: the six tabs, the qualifying & "does not qualify" result cards, the **return-of-capital split in the distributions sub-line**, the portfolio grid for lots, the editable per-distribution ROC column, the saved-ticker pick list, the Info tab (coverage summary, tracked-fund list, CSV links), the Diagnostics scenarios, validation, the fetch-error messages (server / unknown symbol / network), and saved-input restoration. |
