...
def _nearest_amount(divmap, iso, tol_days=6):
    """The dividend amount nearest [iso] within [tol_days] (ex-date ≈ payable),
    for dollar-weighting; 1.0 if none close (equal weight)."""
    target = date.fromisoformat(iso)
    best, best_d = None, tol_days + 1
    for d, amt in divmap.items():
        dist = abs((date.fromisoformat(d) - target).days)
        if dist < best_d:
            best, best_d = amt, dist
    return best if best is not None else 1.0


def compute_annual(history, dividends, current_year, min_count=4):
    """Per-fund, per-COMPLETED-calendar-year ROC% — the dollar-weighted mean of
    that year's 19a-1 per-distribution estimates. The single figure to apply to a
    finished year (where the weekly estimates have settled), vs the live per-
    distribution values for the current year. Emitted only when a year has at
    least [min_count] notices so the average is representative."""
    out = {}
    for t, dates in history.items():
        by_year = {}
        for iso, roc in dates.items():
            y = int(iso[:4])
            if y >= current_year:
                continue
            w = _nearest_amount(dividends.get(t, {}), iso)
            by_year.setdefault(y, []).append((roc, w))
        ann = {}
        for y, pts in by_year.items():
            if len(pts) >= min_count:
                den = sum(w for _, w in pts)
                if den:
                    ann[y] = round(sum(r * w for r, w in pts) / den, 1)
        if ann:
            out[t] = ann
    return out


def write_annual_dart(annual, as_of, dest):
    """lib/roc_annual.dart — {ticker: {year: ROC%}} for completed years. YieldMax
    Form 8937 actuals (when present) override the 19a aggregate upstream."""
    lines = [
    ...
    for t in sorted(annual):
        entries = ", ".join(f"{y}: {annual[t][y]}" for y in sorted(annual[t]))
        lines.append(f"  '{t}': {{{entries}}},")
    lines.append("};")
    with open(dest, "w") as f:
        f.write("\n".join(lines) + "\n")
